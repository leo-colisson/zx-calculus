...
\begin{center}
  ...
  \vspace{1em}
  {Léo Colisson \quad Version 2021/10/27}\\[3mm]
...
\begin{stylekey}{/tikz/defaultEnv/amp}
  Due to \LaTeX{} limitations, |&| can't be used inside macros, and the macro |\zx| does not work in the |align| environment. To solve that, in macros either use the |ampersand replacement=\&| option (whose shortcut is |amp|) or |\zxAmp| (in any case, replace |&| with |\&|). In the |align| environment, don't use |\zx| (other math environments don't have this limitation for what I tried):
\begin{codeexample}[vbox]
An aligned equation:
\begin{align}
  \zxAmp{\zxZ{} \arrow[r] \& \zxX{}} &= \begin{ZX}[amp] \zxX{} \arrow[r] \& \zxZ{} \end{ZX}
\end{align}
% In a macro use \setlength{\fboxsep}{0pt} % removes padding around fbox
% \fbox{\zxAmp{\zxZ{} \rar \& \zxX{}}} or
% \fbox{%
%   \begin{ZX}[amp] % amp is a shortcut for "ampersand replacement=\&"
%     %  See \ here ---v
%     \zxX{} \arrow[r] \& \zxZ{}
%   \end{ZX}%
% }
...
When the top left cell is empty, you may get an error at the compilation |Single ampersand used with wrong catcode| (this error should be fixed in latest released) or |<symbol> allowed only in math mode| (not sure why). To solve this issue, you can add an empty node on the very first cell:
\begin{codeexample}[]
\begin{ZX}
...
You may also get the error |Single ampersand used with wrong catcode| when |&| has already a different meaning, for instance in |align|, in that case you may change the |&| character into |\&| using |[ampersand replacement=\&]|:
\begin{codeexample}[vbox]
\begin{align}
