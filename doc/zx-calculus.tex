...
\begin{center}
  ...
  \vspace{1em}
  {Léo Colisson \quad Version 2022/02/09}\\[3mm]
...
When writing equations, you may also want to change the baseline to align properly your diagrams on a given line like that (|a=blabla| gives the alias name |blabla| to the node, and configure tools useful for debugging):
\begin{codeexample}[]
...
\begin{pgfmanualentry}
  \def\extrakeytext{style, }
  \extractcommand\zxGround\oarg{parent style}\marg{picture style}\@@
  \extractcommand\zxGroundCreate\oarg{parent style}\marg{picture style}\@@
  \extractcommand\zxGroundAbove\oarg{parent style}\marg{picture style}\@@
  \extractcommand\zxGroundBelow\oarg{parent style}\marg{picture style}\@@
...
\paragraph{Add margin around nodes.} You might want to add empty paths to increase the ``margin'' around your picture, or |overlay| some nodes if you don't want them to be counted when computing the bounding box. For instance, below we increase the box by 1cm on each side on the x axis, and by 3mm on each side on the y axis:
{\catcode`\|=12 % Ensures | is not anymore \verb|...|
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyNode}{
    \node[draw,fill=blue!50,draw=blue,inner sep=3mm, name=] at (0,0) {};
    % Empty path to increase the bounding box, just to add some space around the node:
    \path ([xshift=-1cm,yshift=3mm]current bounding box.north west)
          rectangle ([xshift=1cm,yshift=-3mm]current bounding box.south east);
  }
  \begin{ZX}
