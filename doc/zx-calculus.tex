...
\begin{pgfmanualentry}
  \extractcommand\zxCoordinateMain\opt{\oarg{coordinate style}}\marg{}\@@
  \extractcommand\zxCoordinateSubnode\opt{\oarg{coordinate style}}\marg{name subnode}\@@
  \pgfmanualbody
  (starting from 2023-10-03) Since coordinates expect their name to be given in the argument, we cannot use |zx main node| or |zx subnode| on them: these two commands are the equivalent of these styles for coordinates. Note that this is mostly useful if you start to build your own nodes: this way you can easily place the anchors followed by |bezier|-based wires (|fake center {east,north,south,west}|) and |C|-based wires (|true {east,north,south,west}|): For instance, we show how to create a node such that bezier arriving from right will be connected to the south east part of the node while bezier leaving from the left will start at the north east anchor:
\begin{codeexample}[width=0pt]
...
\begin{pgfmanualentry}
  \def\extrakeytext{style, }
  \extractcommand\zxSlice\opt{\oarg{style label}}\opt{\oarg{style line}}\marg{text}\@@
  \extractcommand\zxVSlice\opt{\oarg{style label}}\opt{\oarg{style line}}\marg{text}\@@
  \makeatletter
  \def\extrakeytext{style, }
...
\begin{pgfmanualentry}
  ...
  \extractcommand\wire\marg{options}\@@
  \pgfmanualbody
