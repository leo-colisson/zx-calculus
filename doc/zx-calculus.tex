...
\textbf{Tip}: if you are unsure of the definition of a style in an example, just click on it, a link will point to its definition.
...
\begin{pgfmanualentry}
  ...
  \extractkey/zx/args/-andL/1-\@nil%
  \extractkey/zx/args/-andL/2-\@nil%
  \extractkey/zx/args/-andL/L\@nil%
  \extractkey/zx/args/-andL/1L\@nil%
  \extractkey/zx/args/-andL/2L\@nil%
...
\begin{codeexample}[width=3cm]
  ...
  \end{ZX}
\end{codeexample}
It is possible to configure it using the options in |-andL config| described in \cref{subsec:wirecustom}, where |-| is the (relative) position of the horizontal Bezier anchor and |L| its relative vertical position (to keep a |s|-shape, you should have |-|$>$|L|).
\begin{codeexample}[width=3cm]
\begin{ZX}
  \zxX{\alpha} \ar[rd,s.] \\
  & \zxZ{\beta}\\
...
\begin{pgfmanualentry}
  \makeatletter
  \def\extrakeytext{style, }
  \extractkey/zx/wires definition/N=angle (default defaultN)\@nil%
  \extractkey/zx/wires definition/N'=angle (default defaultN)\@nil%
  \extractkey/zx/wires definition/N.=angle (default defaultN)\@nil%
  \extractkey/zx/wires definition/-N=angle (default \{defaultN,defaultN-\})\@nil%
  \extractkey/zx/wires definition/-N'=angle (default \{defaultN,defaultN-\})\@nil%
  \extractkey/zx/wires definition/-N.=angle (default \{defaultN,defaultN-\})\@nil%
  \extractkey/zx/wires definition/N-=angle (default \{defaultN,defaultN-,symmetry\})\@nil%
  \extractkey/zx/wires definition/N'-=angle (default \{defaultN,defaultN-,symmetry\})\@nil%
  \extractkey/zx/wires definition/N.-=angle (default \{defaultN,defaultN-,symmetry\})\@nil%
  \makeatother
  \pgfmanualbody
...
\begin{codeexample}[width=3cm]
  ...
  \end{ZX}
\end{codeexample}
Like other wires, it can be configured using |-| (horizontal relative position of anchor points) and |L| (vertical relative position of anchor points)\dots{} (make sure to have |-|$<$|L| to have a |N|-looking shape).
\begin{codeexample}[width=3cm]
  \begin{ZX}
    \zxX{\alpha} \ar[N,rd] \\
                               & \zxZ{\beta}\\
...
\begin{pgfmanualentry}
  \makeatletter
  \def\extrakeytext{style, }
  \extractkey/zx/wires definition/NN=angle (default \{defaultN,symmetry-L,defaultNN\})\@nil%
  \extractkey/zx/wires definition/NN.=angle (default \{defaultN,symmetry-L,defaultNN\})\@nil%
  \extractkey/zx/wires definition/.NN=angle (default \{defaultN,symmetry-L,defaultNN\})\@nil%
  \extractkey/zx/wires definition/NIN=angle (default \{defaultN,symmetry-L,defaultNN,defaultNIN\})\@nil%
  \extractkey/zx/wires definition/INN=angle (default \{defaultN,symmetry-L,defaultNN,defaultNIN,symmetry\})\@nil%
  \extractkey/zx/wires definition/NNI=angle (default \{defaultN,symmetry-L,defaultNN,defaultNIN,symmetry\})\@nil%
  \makeatother
  \pgfmanualbody
...
\begin{pgfmanualentry}
  ...
  \makeatother
  \pgfmanualbody
  Forces the wires to start at the center of the node (|wire centered start|, alias |wcs|), to end at the center of the node (|wire centered end|, alias |wce|) or both (|wire centered|, alias |wc|). This may be useful, for instance in the old |IO| mode when nodes have different sizes (the result looks strange otherwise), or with some wires (like |C|) connected to |\ZxNone+| (if possible, use |\zxNone| (without any embellishment) since it does not suffer from this issue as it is a coordinate).
...
\begin{pgfmanualentry}
  \makeatletter
  \def\extrakeytext{style, }
  \extractkey/zx/wires definition/bezier=\{x1\}\{y1\}\{x2\}\{y2\} \@nil%
  \extractkey/zx/wires definition/bezier x=\{x1\}\{y1\}\{x2\}\{y2\} \@nil%
  \extractkey/zx/wires definition/bezier y=\{x1\}\{y1\}\{x2\}\{y2\} \@nil%
  \makeatother
  \pgfmanualbody
...
\begin{pgfmanualentry}
  ...
  \pgfmanualbody
  These are spaces, to use like |&[\zxHCol]| or |\\[\zxHRow]| in order to increase the default spacing of rows and columns depending on the style of the wire. |H| stands for Hadamard, |S| for Spiders, |W| for Wires only, |w| is you link a |zxNone| to a spider, |HS| for both Spiders and Hadamard, |Dots| for the 3 dots styles. And of course |Col| for columns, |Row| for rows.
\end{pgfmanualentry}
