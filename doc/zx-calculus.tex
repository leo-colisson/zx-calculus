...
\zxNewNodeFromPic{MyDivider}[][zx create anchors={\zxVirtualCenterWest, \zxVirtualCenterEast},
  ...
  \node[regular polygon, regular polygon sides=3,shape border rotate=90,%shape border rotate=90,
        draw=black,fill=gray!50, inner sep=1.6pt, rounded corners=0.8mm,name=] {};
...
\begin{stylekey}{/tikz/defaultEnv/amp}
...
\begin{pgfmanualentry}
  \def\extrakeytext{style, }
  \extractcommand\zxGround\opt{.}\opt{-}\opt{'}\oarg{picture style}\marg{}\@@
...
\begin{pgfmanualentry}
  \def\extrakeytext{style, }
  \extractcommand\zxDivider\opt{.}\opt{-}\opt{'}\marg{}\@@
  \pgfmanualbody
...
\begin{pgfmanualentry}
  \def\extrakeytext{style, }
  \extractcommand\zxMatrix\opt{.}\opt{-}\opt{'}\opt{/}\opt{*}\opt{\_\{text in pmatrix\}}\oarg{pic style}\oarg{node style}\marg{matrix name}\@@
  \pgfmanualbody
...
The |*| option is used to reverse the direction of the arrow, typically for the transpose (note how we can give an alias (with |a| in |node style|) to combine with |math baseline| to properly vertically align the node):
{\catcode`\|=12 % Ensures | is not anymore \verb|...|
\begin{codeexample}[width=0pt]
We define %
$\begin{ZX}[math baseline=myf]
  \zxN{} \rar[B] & \zxMatrix*[][a=myf]{A} \rar[B] & \zxN{}
\end{ZX} \coloneqq
\begin{ZX}[math baseline=myA]
            & [\zxWCol] \zxN{} \ar[B,dr,s]                 & [\zxWCol] \zxN{} \\[\zxZeroCol+.3mm]
\ar[B,dr,s] & \zxMatrix[][a=myA]{A} \ar[B,u,C] \ar[B,d,C-] &                  \\
            &                                              & \zxN{}           \\
\end{ZX}$.
\end{codeexample}
}
...
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyNode}{
    \node[draw,fill=blue!50,draw=blue,inner sep=3mm, name=, alias=mynode] at (0,0) {};
...
Note how giving an empty name |name=| to a node makes it the target of arrows. Without this, the lines would go to the coordinate |(0,0)|, and go through all the nodes:
{\catcode`\|=12 % Ensures | is not anymore \verb|...|
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyNode}{
...
We can similarly put the |name=| on the other node to point to the other node by default:
{\catcode`\|=12 % Ensures | is not anymore \verb|...|
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyNode}{
    \node[draw,fill=blue!50,draw=blue,inner sep=3mm, alias=mynode] at (0,0) {};
    \node[draw,fill=red!50,draw=blue,inner sep=1mm, name=] at (mynode.north east) {};
  }
  \begin{ZX}
...
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyNode}{
    \node[draw,fill=blue!50,draw=blue,inner sep=3mm, name=] at (0,0) {};
...
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyNode}{
    \node[draw,fill=blue!50,draw=blue,inner sep=3mm, name=, alias=mynode] at (0,0) {\tikzpictext};
    \node[draw,fill=red!50,draw=blue,inner sep=1mm] at (mynode.north east) {};
  }
  \begin{ZX}
...
\zxNewNodeFromPic{MyBox}[
  ...
  \node[draw, inner sep=1.3mm, rectangle, name=, execute at begin node=$, execute at end node=$,
...
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyTriangle}[
    ...
    scale=1.3
      ...
      \node[regular polygon, regular polygon sides=3, shape border rotate=90, draw=black,fill=blue!50, inner sep=1mm,rounded corners, name=] {};
  }
  \begin{ZX}
...
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyTriangle}[
    every node/.style={transform shape},
      ...
      \node[regular polygon, regular polygon sides=3, shape border rotate=90, draw=black,fill=blue!50, font=\footnotesize,inner sep=1pt,rounded corners, name=] {\tikzpictext};
  }
  \begin{ZX}
...
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyTriangle}[
    ...
    \node[
      ...
      inner sep=1pt,rounded corners, name=,
...
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyStar}[][
    ...
    zx create anchors={anchorA,anchorB},
      ...
      \node[star, minimum size=6mm, draw, alias=mystar, fill=red, name=] at (0,0) {};
...
\begin{codeexample}[width=0pt]
  \zxNewNodeFromPic{MyDoubleNode}{
      \node[fill=blue!20, name=, inner sep=3mm] at (0,0) {};
