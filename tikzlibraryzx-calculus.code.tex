...
\def\zx@message#1{}
\def\zx@verbose@message#1{\message{#1}} % For more important messages. Like to show the code is not looping forever
% To enable debugging:
% \def\zx@message#1{\message{#1}}
      ...
      \pgfintersectionofpaths{%
        ...
        \begin{pgfinterruptboundingbox}%
          ...
          \else%
            ...
            %% Parse first point to get coordinates in \pgf@x and y
            ...
            \fi%
            \edef\zx@tmp@xa{\the\pgf@x}%
            \edef\zx@tmp@ya{\the\pgf@y}%
            \ifnum#5=1 \relax%
              \tikz@scan@one@point\pgf@process(#3)%
            \else%
              \tikz@scan@one@point\pgf@process(#2)%
            \fi%
            \edef\zx@tmp@xb{\the\pgf@x}%
            \edef\zx@tmp@yb{\the\pgf@y}%
            \pgfkeysalso{/zx/tmp/zx@getControlPoints={\zx@tmp@xa}{\zx@tmp@ya}{\zx@tmp@xb}{\zx@tmp@yb}}%
            ...
            \pgf@process{%
              \pgfqpointscale{10}{%
                \pgfpointdiff{\tikz@scan@one@point\pgf@process(\zx@control)%
                }{%
        ...
        \pgfextra{% <- Type latex code inside path
          ...
          \pgfkeysalso{
            ...
            /zx/tmp/zx@getControlPoints/.code n args={4}{
              ...
              % Use ########1 ... (yes, LaTeX is a funny language, and I love nested functions)
