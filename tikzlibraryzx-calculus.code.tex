...
\NewDocumentCommand{\zxNewNodeFromPic}{mO{}O{}O{}O{}m}{%
  ...
  % }%
  % The t are for modes
  \ExpandArgs{c}\NewExpandableDocumentCommand{zx#1}{t.t-t't/t*e_O{}O{}m}{%
    |[#1={
      pic text={##9},
...
\zxNewNodeFromPic{Divider}[][zx create anchors={\zxVirtualCenterWest, \zxVirtualCenterEast},
  every node/.style={transform shape}
]{
  \node[regular polygon, regular polygon sides=3,shape border rotate=90, draw=black,fill=gray!50, inner sep=1.6pt, rounded corners=0.8mm,name=] {};
...
\zxNewNodeFromPic{Matrix}
  ...
  \node[name=, draw, signal, fill=colorZxMatrix, inner sep=1pt, minimum height=6pt, transform shape,
...
%   \node[anchor=center] at (fake center west) {x};
% }

\zxNewNodeFromPic{Box}[][zx create anchors={fake center north,fake center south,fake center east,fake center west,bla}]{%
  \node[draw, inner sep=1.3mm, rectangle, name=bli, execute at begin node=$, execute at end node=$,outer sep=0pt,
    /zx/picCustomStyleBoxMainNode/.append style={}, % Make sure it exists to avoid errors
    /zx/picCustomStyleBoxMainNode,
    ] at (0,0) {\tikzpictext};%
  \coordinate(fake center north) at (bli.north);%
  \coordinate(fake center west) at (bli.west);%
  \coordinate(fake center east) at (bli.east);%  
  \coordinate(fake center south) at (bli.south);%
  \node[anchor=center,name=bla] at (fake center west) {x};
...
\tikzset{fit margins/.style={/tikz/afit/.cd,#1,
    ...
    xshift=-\pgfkeysvalueof{/tikz/afit/left}+\pgfkeysvalueof{/tikz/afit/right},
    yshift=-\pgfkeysvalueof{/tikz/afit/bottom}+\pgfkeysvalueof{/tikz/afit/top}},
    afit/.cd,left/.initial=1pt,right/.initial=1pt,bottom/.initial=1pt,top/.initial=1pt,/tikz/.cd}
