...
\tikzset{
  ...
  /zx/defaultEnv/.style={
    nodes in empty cells=true, %% To avoid errors about single ampersand when matrix does not start with None.
...
%%%%% Like \zx but uses \& instead of &. This is useful for instance in "align" environments
%%%%% since \zx{} will given an error (even without using &, changing the catcode is enough to
%%%%% break the function). However, \begin{ZX}...\end{ZX} seems to work in align without any issues here
%%%%% but in other cases it can still cause troubles (like in \fbox{}) if you use &. So either use
