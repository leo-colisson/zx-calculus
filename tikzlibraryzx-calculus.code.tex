...
\NewExpandableDocumentCommand{\zxMainCoordinate}{O{}m}{
...
\NewExpandableDocumentCommand{\zxGateMulti}{O{}mmm}{
  ...
  \path;\pgfkeys{
    ...
    fit content/.code n args={4}{
      ...
      \end{lrbox}%
      % \sbox{\zx@tmp@box}{#4 xxxx}%
      % \sbox\zx@tmp@box{\begin{tikzpicture}\node{xxx};\end{tikzpicture}}%
      % % \usebox{\zx@tmp@box}
      % \message{\the\dimexpr\the\ht\zx@tmp@box\relax}
      % % we measure the content of the box (2 times since we have left and right margins):
...
\NewExpandableDocumentCommand{\zxOCtrl}{O{}m}{%
  \node[zx main node, circle, inner sep=2pt, draw, #1]{};
}

\NewExpandableDocumentCommand{\zxNot}{O{}m}{%
  \node[zx main node, circle, inner sep=2.5pt, draw, append after command={(\tikzlastnode.north) edge (\tikzlastnode.south) (\tikzlastnode.east) edge (\tikzlastnode.west)}, #1]{};
}

\NewExpandableDocumentCommand{\zxCross}{O{}m}{%
  \zxMainCoordinate{}
  \node[at={(0,0)}, rectangle, inner sep=3pt, append after command={(\tikzlastnode.north east) edge[line cap=round] (\tikzlastnode.south west) (\tikzlastnode.south east) edge[line cap=round] (\tikzlastnode.north west)}, #1]{};
}
