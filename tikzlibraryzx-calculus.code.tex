...
% \end{ZX}
%
% - Understand why it is not possible to do: \ar[to=buttons,end subnode=redNode] with alias like \zxMyDoubleNode[a=buttons]{}
% 
...
\tikzset{
  /zx/wires definition/.style={
    ...
    Bn'Args/.style 2 args={bold, "/" {anchor=center,##2}, "##1" {above,yshift=3pt,##2}},
    Bn'Args/.default={n}{},
    Bn.Args/.style 2 args={bold, "/" {anchor=center,##2}, "##1" {below,yshift=-3pt,##2}},
    Bn.Args/.default={n}{},
    %% for wires top/down (left,right)
    BnArgs/.style 2 args={bold, "/" {rotate=90,anchor=center,##2}, "##1" {left,xshift=-2pt,##2}},
    BnArgs/.default={n}{},
    Bn-Args/.style 2 args={bold, "/" {rotate=90,anchor=center,##2}, "##1" {right,xshift=2pt,##2}},
    Bn-Args/.default={n}{},
    ...
    start subnode/.code={\edef\tikzcd@ar@start{\tikzcd@ar@start##1}},
    end subnode/.code={\edef\tikzcd@ar@target{\tikzcd@ar@target##1}},
  ...
  /zx/defaultEnv/.style={
    ...
    math baseline row/.style={math baseline=\tikzcdmatrixname-##1-1},
      ...
      a/.code={%
        \pgfkeysalso{%
          ...
          append after command={\pgfextra{%
              ...
              % manually
              \expanded{\noexpand\gdef\csname zx@alias@\zxCurrentDiagram @####1\endcsname{\tikzlastnode}}%
            }%
          },
        }%
...
\tikzset{
  ...
  zx subnode/.style={
    name=\tikzcdmatrixname-\zxOriginalRow-\zxOriginalCol#1,%
  },
  zx independent subnode/.style={
    name=\tikzcdmatrixname-\the\pgfmatrixcurrentrow-\the\pgfmatrixcurrentcolumn#1,%
  },
}
...
% \zxExecuteAtEndPicture{\node{A};} will execute \node{A}; at the end of the picture
% It will automatically define \zxOriginalRow, \zxOriginalCol and
% \zxGetNameRelativeNode{n}{m} to get the row/col where this command was run
% and to get the position of the relative node relative to the position of the command
...
\NewExpandableDocumentCommand{\zxExecuteAtCellAbsolute}{mmm}{%
  \def\zx@tmp@args{#3}%
  \edef\zx@tmp@code{
    \noexpand\expandafter\noexpand\gdef\noexpand\csname zxExecuteAtCell-\tikzcdmatrixname-#1-#2\noexpand\endcsname{%
      \noexpand\def\noexpand\zxOriginalRow{\the\pgfmatrixcurrentrow}%
      \noexpand\def\noexpand\zxOriginalCol{\the\pgfmatrixcurrentcolumn}%
      \expandonce{\zx@tmp@args}%
    }%
...
\NewExpandableDocumentCommand{\zxSetVariable}{mm}{%
  ...
  \edef\zx@tmp{zxGateMultiVariable-\tikzcdmatrixname-\the\pgfmatrixcurrentrow-\the\pgfmatrixcurrentcolumn-#1}%
  \expandafter\gdef\csname \zx@tmp\endcsname{%
    #2%
  }%
  \expandafter\show\csname zxGateMultiVariable-\tikzcdmatrixname-\the\pgfmatrixcurrentrow-\the\pgfmatrixcurrentcolumn-#1\endcsname
}

\NewExpandableDocumentCommand{\zxGetVariable}{m}{%
  \csname zxGateMultiVariable-\tikzcdmatrixname-\zxOriginalRow-\zxOriginalCol-#1\endcsname%
}
...
\NewDocumentEnvironment{ZXNoExt}{O{}}{%
  ...
  \else% Copy/pasted from https://github.com/astoff/tikz-cd/blob/master/tikzlibrarycd.code.tex, just added "every arrow post".
    ...
    \ifdefined\zxUsePatchCmdToPatchArrows%
      ...
      {\path[/tikz/commutative diagrams/.cd,everyx arrow,#1]}%
...
\zxNewNodeFromPic{Box}[
  ...
  \node[draw, inner sep=1.3mm, rectangle, zx main node, execute at begin node=$, execute at end node=$,alias=hello,
