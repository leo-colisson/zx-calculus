...
\def\zxShouldUseIntersections{}
% to disable this check, undefined it:
% \let\zxShouldUseIntersections\undefined
...
%%% I can't find in pgfkeys a way to store coordinates: pgfpoint simply redefines
%%% \pgx@x and \pgf@y, which get overwritten quickly...
%%% Usage: \edef\abc{6pt,2pt}
%%% Hello \zx@parse@coordx{(\abc)}.
%%% We use this syntax since \tikztolast uses it (\tiztolast) -- is replaced by (6pt,2pt) --.
%%% Make sure to use edef, or the coordinates may be changed later if 6pt is replaced by a macro.
\def\zx@parse@coordx@tmp(#1,#2){#1}%
\def\zx@parse@coordx#1{%
  \expandafter\zx@parse@coordx@tmp\expandafter#1
}
\def\zx@parse@coordy@tmp(#1,#2){#2}%
\def\zx@parse@coordy#1{%
  \expandafter\zx@parse@coordy@tmp\expandafter#1
}
...
\def\zx@find@intersection@fakecenter#1#2#3#4{%
  % First, we check if the user wants to compute the intersection:
  \ifdefined\zxShouldUseIntersections%
    ...
    \ifcsname tikz@intersect@path@name@\tikz@intersect@path@a \endcsname%
      \message{ZX: the intersection was found :-)}
      \pgfintersectionofpaths{%
        ...
        \pgfextractx{\pgf@xa}{\pgfpointdiff{\pgfpoint{\zx@parse@coordx{(#2)}}{\zx@parse@coordy{(#2)}}}{\pgfpointanchor{#1}{center}}}%
        \pgfextracty{\pgf@ya}{\pgfpointdiff{\pgfpoint{\zx@parse@coordx{(#2)}}{\zx@parse@coordy{(#2)}}}{\pgfpointanchor{#1}{center}}}%
        \pgfpathmoveto{\pgfpoint{-\pgf@xa}{\pgf@ya}}%
        %% What, now both axis are inverted??? If someone can explain me this...
        \pgfextractx{\pgf@xa}{\pgfpointdiff{\pgfpoint{\zx@parse@coordx{(#3)}}{\zx@parse@coordy{(#3)}}}{\pgfpointanchor{#1}{center}}}%
        %\message{HUUU what's going on}
        %\show#3
        \pgfextracty{\pgf@ya}{\pgfpointdiff{\pgfpoint{\zx@parse@coordx{(#3)}}{\zx@parse@coordy{(#3)}}}{\pgfpointanchor{#1}{center}}}%
      ...
      \ifnum\pgfintersectionsolutions=1%% The good number of solution has been found!
        \message{[ZX] Cool, just found one solution!}
      ...
      \else % The bad number of solution has been found
        \message{[ZX] WARNING: expecting one solution, found \pgfintersectionsolutions}
      ...
      \fi
    \else%
      \message{ZX: no path named \tikz@intersect@path@a was found.}%
      \ifdefined\zxWireInsideIfNoIntersectionName %
        % No intersection was found, but still want to use fake center (or center).
        % ==> The wire will be inside the shape
        % Basically \tikztostart := tikztostart@new@center
    ...
    \fi%
  \else%
      \message{[ZX]: intersection mechanism disabled}%
      \ifdefined\zxWireInsideIfNoIntersectionName %
        % No intersection was found, but still want to use fake center (or center).
        % ==> The wire will be inside the shape
        % Basically \tikztostart := tikztostart@new@center
...
\tikzset{
  /zx/wires definition/.style={
    ...
    % See explaination at the beginning where we define \def\zxShouldUseIntersections{}
    ...
    bezier@general/.style n args={6}{
      ...
      to path={
        ...
        \pgfextra{% <- Type latex code inside path
          ...
          \message{[ZX] The start anchor is \zx@anchor@start@node}%
          \message{[ZX] The end anchor is \zx@anchor@end@node}%
          ...
          \ifthenelse{\equal{\zx@anchor@start@node}{}}{%
            ...
            \ifthenelse{\equal{\zx@anchor@end@node}{}}{%
              ...
              \message{[ZX] Both anchors are set. Lets configure intersection.}
              \pgfkeysalso{
    ...
    /zx/utils/change@tikz@fake@center@intersect/.code n args={5}{%
      % ##1: \tikztostart or \tikztotarget
      ...
      \ifPgfpointOrNode{##1}{}{% This is a node, not a point:
        ...
        \message{ZX: coordinate of new center is \zx@new@center}%
        \pgfkeysalso{%
          /zx/utils/change@tikz@fake@center={##3}{##4}{\zx@new@center@other},%
        }%
        \message{ZX: coordinate of new center is \zx@new@center}%
        \message{ZX: coordinate of new other center is \zx@new@center@other}%
        \zx@find@intersection@fakecenter{##1}{\zx@new@center}{\zx@new@center@other}{##5}%
      }%
    },
